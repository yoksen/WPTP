...
def train_b50(model, writer, train_loader, optimizer, criterion, epoch, task_idx, linear, linear_optim, simclr_aug, data_loader=None):
    model.zero_grad()
    model.train()
    if task_idx != 0:
        num_cls = args.output_size
...
def train_joint_b50(model, joint_linear, criterion, optimizer, scheduler, loader, simclr_aug, task_id):
    model.eval()
    loss_joint_cum = 0
    if task_id == 0:
        num_cls = 50
